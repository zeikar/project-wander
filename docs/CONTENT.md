...
Arrival now names the most recent thing left standing on a leg that held two — the LEG, verbatim, and the KIND, never a species. That borrows the shape of a road sign (§ *Forks*: a sign names a KIND, never a species) but not its reason: a sign buys almost nothing by naming the species, while here the codex persists across journeys, so a named species at arrival would hand the next run a checklist. One nullable state field holds it — a leg index and a kind, overwritten by each later pair — never an array. It is one sentence, the most recent thing only, never a count and never a list: a list is a ledger, and a ledger read at arrival is a completion counter, which `VISION.md` § *Success Metric* refuses. A journey that passed nothing by gets no line, not a fallback one. The arrival GRADE is untouched by it, pinned by a test that fails if the grade ever reads what was left standing.

This is UNMEASURED — a hypothesis for the next human playtest, not a result. A golden-trace re-run of 100 seeded journeys, before and after, comes back byte-identical once the new field is removed (1403 state rows, 704 of them non-null): a no-regression proof about the change and no more than that — it says nothing about whether the line helps anyone.
